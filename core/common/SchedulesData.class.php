...
class SchedulesData extends CodonData {
    ...
    public static function getfavac($pilotid) {
        $pilotid = DB::escape($pilotid);
        $sql = 'SELECT `aircraft`,
                COUNT(`aircraft`) AS `total` 
                FROM     ' . TABLE_PREFIX . 'pireps WHERE `pilotid`='.$pilotid.'
                GROUP BY `aircraft`
                ORDER BY `total` DESC
                LIMIT    1';
        ...
        $ip = $result->aircraft;
        $k = OperationsData::getAircraftInfo($ip);
        $total = $k->icao;
    ...
    public static function countBids($pilotid) {
        $pilotid = DB::escape($pilotid);
        $sql = 'SELECT COUNT(*) AS total FROM ' . TABLE_PREFIX . 'bids
    ...
    public static function performintro($pilotid, $intro) {
        $pilotid = DB::escape($pilotid);
    ...
    public static function getRecentSchedules($count){
        $sql = 'SELECT * FROM ' . TABLE_PREFIX . 'schedules ORDER BY id DESC LIMIT '.$count.'';
        return DB::get_results($sql);
    }
...
AND YEAR(submitdate) = YEAR(CURRENT_DATE()) GROUP BY pilotid ORDER BY count DESC';
    ...
    public static function addemailBid($pilotid, $routeid){
        $pilotid = DB::escape($pilotid);
        $routeid = DB::escape($routeid);
        
        $sql = 'INSERT INTO phpvms_bids (pilotid, routeid, dateadded)
				VALUES (' . $pilotid . ', ' . $routeid . ', NOW())';
        DB::query($sql);
        
        $pilotdet = PilotData::getPilotData($pilotid);
        $schedule = SchedulesData::getSchedule($routeid);
        
        
        $pilotname = $pilotdet->firstname.' '.$pilotdet->lastname;
        $pilotemail = $pilotdet->email;
        $subject = SITE_NAME.' : Flight Booking!';
        $message = '
        <div style="border: 2px solid #46143c; border-radius:25px;width: 500px;
    margin: 0 auto;padding:30px;">
<table style="padding:20px; ">
    <tr style="padding:5px">
    <td style="text-align:left;" colspan="2"><img style="width:250px; height: 50px" src="http://vistaravirtual.com/images/logo-dark-275x55.png"></td>
    </tr>
    <tr>
<td style="font-weight:bold;"><span style="padding-top:15px;margin-top:15px"><br><br>Pilot in Command : '.$pilotname.'</span><br>
                                
                                Current Hours : '.$pilotdet->totalhours.' hrs<br>
                                <img style="float:right" src="http://vistaravirtual.com/unnamed.png"></td>
</tr>
    <tr>
    <td colspan="2" style="font-weight:bold">Flight Itenary :</td>
    </tr>
    
       <tr>
    <table style="padding:20px; border: 1px solid black; width:100%;" border="1">
        <thead>
            <th style="border: 1px solid black; padding:2px">Departure</th>
            <th style="border: 1px solid black">Arrival</th>
            <th style="border: 1px solid black">Flight</th>
            <th style="border: 1px solid black">Dep Time</th>
            <th style="border: 1px solid black">Arr Time</th>
            <th style="border: 1px solid black">Distance</th>
            <th style="border: 1px solid black">Aircraft</th>
            <th style="border: 1px solid black">Flight Time</th>
        </thead>
        <tbody>
        <tr>
            <td>'.$schedule->depicao.'</td>
		    <td>'.$schedule->arricao.'</td>
            <td>'.$schedule->flightnumber.'</td>
            <td>'.$schedule->deptime.'</td>
            <td>'.$schedule->arrtime.'</td>
            <td>'.$schedule->distance.' nm</td>
            <td>'.$schedule->aircraft.'</td>
            <td>'.$schedule->flighttime.' hrs</td>
<tr>
            <td colspan="8" style="color:red; font-weight:bold; border:none; text-align:center"><a style="color:red;" href="'.SITE_URL.'/index.php/action.php/schedules/boardingpass/'.$routeid.'?newwindow">Get Boarding Pass</a></td></tr> 
            <tr>
                <td colspan="8" style="color:red; font-weight:bold; border:none; text-align:center"><a href="'.SITE_URL.'/index.php/schedules/details/'.$routeid.'">Click here to view more Details</a></td></tr> 
            <tr>
                <td colspan="8" style="color:#46143c; font-weight:bold; border:none; text-align:center">Bon Voyage Captain!</td></tr>        </tbody>
    </table></tr>
    ...
    public static function getdepapts(){
    ...
    public static function getarrapts(){
        $sql ="SELECT s.arricao, a.name as arrname FROM ".TABLE_PREFIX."schedules s
	        LEFT JOIN ".TABLE_PREFIX."airports a ON a.icao = s.arricao
			LEFT JOIN ".TABLE_PREFIX."bids b ON b.routeid = s.id
			LEFT JOIN ".TABLE_PREFIX."aircraft ac ON ac.id = s.aircraft
			";
        return DB::get_results($sql);
